<style>
  ...
  .active {
    background-color: #33d457 !important;
  }
</style>
...
<aside class="main-sidebar sidebar-navy-primary bg-navy elevation-4 sidebar-no-expand">
  ...
  <a href="<?php echo base_url ?>admin" class="brand-link bg-primary text-sm">
    <img src="<?php echo validate_image($_settings->info('logo')) ?>" alt="Store Logo"
      class="brand-image img-circle elevation-3 border-1"
...
<script>
  $(document).ready(function () {
    ...
    if ($('.nav-link.nav-' + parent).length > 0) {
      $('.nav-link.nav-' + parent).addClass('active')

      if ($('.nav-link.nav-' + parent).hasClass('tree-item') == true) {
        $('.nav-link.nav-' + parent).closest('.nav-treeview').siblings('a').addClass('active')
        $('.nav-link.nav-' + parent).closest('.nav-treeview').parent().addClass('menu-open')
      }
      if ($('.nav-link.nav-' + parent).hasClass('nav-is-tree') == true) {
        $('.nav-link.nav-' + parent).parent().addClass('menu-open')
      }
    } else {
      $('.nav-link.nav-' + parent + '-' + sibling).addClass('active')
    }
